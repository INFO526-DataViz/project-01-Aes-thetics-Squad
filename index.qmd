...
Understanding the variations in tornado intensity is vital for assessing their potential impact and devising effective disaster response plans. In this section, we explore the patterns in tornado magnitude concerning geographical location (states) and how they fluctuate across different times of the year. The variables that are necessary to answer this question are yr-year in which the tornado occurred, st-state in which the tornado occurred, slat- starting latitude, slon-starting longitude, mag-magnitude of tornado.

Analyzing whether certain regions or seasons are more prone to producing stronger tornadoes can contribute to a better understanding of tornado dynamics and the factors that influence their intensity. For instance, if there are regions or seasons where stronger tornadoes are more likely, it could lead to targeted preparedness efforts and resources being allocated to those areas. This question is therefore of great interest as it has practical implications for both disaster response and mitigation strategies, ultimately aiming to reduce the impact of tornadoes on communities and save lives.

### Approach

To investigate potential patterns in the magnitude of tornadoes based on location and time of year, we will employ two distinct types of plots: two line plots and a grouped bar plot. The first line plot will depict trends in mean tornado magnitude over the years, offering insights into potential shifts in the intensity of tornadoes. The second line plots the count of tornadoes over time with a trend line to show the overall trend in the plot. The use of a line plots allows for a clear visualization of any long-term variations in tornado magnitudes.

Additionally, we utilized a grouped bar plot to illustrate the distribution of tornado magnitudes across different 12-year intervals. In this plot for each 12 year interval it shows the tornadoes occcurences and the count of the tornadoes for each EF scale rating. This plot is suitable for comparing categorical data across multiple groups, as it provides a visual comparison of tornado occurrences within distinct magnitude categories.
...
The first set of visualizations that is the line plot that is focused on tornado magnitude trends over the years. We observed fluctuations in the mean tornado magnitude per year, with peaks and valleys throughout the decades. We can see that the intensity of tornadoes decreased as we moved forward from 1950 to 2022. From the second plot we can see that the count of tornadoes increased over time. The trend line in the plot shows this increase effectively from 1950 to 2022.
...
For the contour maps, we can see the for the variation in tornado density for the states "Alabama" "Iowa", "Kansas","North Dakota", "Tennessee".
